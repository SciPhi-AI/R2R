...
def wrapper(request):
    ...
    try:
        ...
        # inject the app into the wrapper
        wrapper.app = R2R(config=config)
        yield wrapper
    finally:
...
@pytest.mark.parametrize("wrapper", ["pgvector"], indirect=True)
@pytest.mark.asyncio
async def test_ingest_txt_file_update_file(wrapper):
    # Prepare the test data
