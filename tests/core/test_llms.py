...
# # Fixture for LlamaCPP
# @pytest.fixture
# def llama_cpp():
#     config = LlamaCppConfig(model="tinyllama-1.1b-chat-v1.0.Q2_K.gguf") # , model_path="~/.cache/models")
...
def test_get_completion_ollama(request, llm_fixture):
    ...
    messages = [{"role": "user", "content": "This is a test, return only the word `True`"}]
    generation_config = GenerationConfig(
        model="ollama/llama2",
        temperature=0.0,
        top_p=0.9,
