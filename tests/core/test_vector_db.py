...
from r2r.vector_dbs import (
    LocalVectorDB, LocalVectorDBConfig,
    PGVectorDB, 
    QdrantDB,
)
...
def generate_random_vector_entry(id: str, dimension: int) -> VectorEntry:
    ...
    return VectorEntry(id=generate_id_from_label(id), vector=Vector(vector), metadata=metadata)

dimension = 3
num_entries = 100
sample_entries = [generate_random_vector_entry(f"id_{i}", dimension) for i in range(num_entries)]

# Fixture for LocalVectorDB
@pytest.fixture
def local_vector_db():
    random_collection_name = f"test_collection_{random.randint(0, 1_000_000_000)}"
    config = LocalVectorDBConfig(provider="local", collection_name=random_collection_name)
...
def pg_vector_db():
    random_collection_name = f"test_collection_{random.randint(0, 1_000_000_000)}"
    config = VectorDBConfig(provider="pgvector", collection_name=random_collection_name)
...
def qdrant_vector_db():
    random_collection_name = f"test_collection_{random.randint(0, 1_000_000_000)}"
    config = VectorDBConfig(provider="qdrant", collection_name=random_collection_name)
    db = QdrantDB(config)
    db.initialize_collection(dimension=dimension)
    yield db

# Parameterize the tests to run with both LocalVectorDB and PGVectorDB
@pytest.mark.parametrize("db_fixture", ["local_vector_db", "pg_vector_db", "qdrant_vector_db"])
...
def test_search_with_filters(request, db_fixture):
    ...
    query_vector = sample_entries[0].vector.data
    results = db.search(query_vector=query_vector, filters={"key": filtered_id})
    assert len(results) == 1
    assert results[0].id == sample_entries[0].id
...
def test_upsert(request, db_fixture):
    db = request.getfixturevalue(db_fixture)
    db.upsert(sample_entries[0])
    modified_entry = VectorEntry(id=sample_entries[0].id, vector=Vector([0.5, 0.5, 0.5]), metadata={"key": "new_value"})
