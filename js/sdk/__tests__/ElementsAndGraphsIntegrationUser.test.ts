...
describe("r2rClient V3 System Integration Tests User", () => {
  ...
  let user1GraphId: string;
  let user2GraphId: string;
  ...
  test("Create a graph as user 1", async () => {
    const response = await user1Client.graphs.create({
      name: "Graph 1",
      description: "User 1's first graph",
    });
    expect(response.results).toBeDefined();
    user1GraphId = response.results.id;
    expect(user1GraphId).toEqual(response.results.id);
    expect(response.results.name).toEqual("Graph 1");
    expect(response.results.description).toBe("User 1's first graph");
  });

  test("Create a graph as user 2", async () => {
    const response = await user2Client.graphs.create({
      name: "Graph 1",
      description: "User 2's first graph",
    });
    expect(response.results).toBeDefined();
    user2GraphId = response.results.id;
    expect(user2GraphId).toEqual(response.results.id);
    expect(response.results.name).toEqual("Graph 1");
    expect(response.results.description).toBe("User 2's first graph");
  });
  ...
  // test("User 1 should be able to add entities to their graph", async () => {
  //   const response = await user1Client.graphs.addEntity({
  //     id: user1GraphId,
  //     entityId: user1Entity1Id,
  //   });
  //   expect(response.results).toBeDefined();
  // });
  ...
  // test("User 1 deletes their graph", async () => {
  //   const response = await user1Client.graphs.delete({ id: user1GraphId });
  //   expect(response.results).toBeDefined();
  //   expect(response.results.success).toBe(true);
  // });

  test("User 1 should not be able to delete user 2's graph", async () => {
    await expect(
      user1Client.graphs.delete({ id: user2GraphId }),
    ).rejects.toThrow(/Status 403/);
  });

  // test("User 2 deletes their graph", async () => {
  //   const response = await user2Client.graphs.delete({ id: user2GraphId });
  //   expect(response.results).toBeDefined();
  //   expect(response.results.success).toBe(true);
  // });

  test("REMOVE THIS, BUT A SUPER USER NEEDS TO DELETE THE GRAPHS FOR NOW", async () => {
    const firstResponse = await client.graphs.delete({ id: user1GraphId });
    expect(firstResponse.results).toBeDefined();
    expect(firstResponse.results.success).toBe(true);

    const secondResponse = await client.graphs.delete({ id: user2GraphId });
    expect(secondResponse.results).toBeDefined();
    expect(secondResponse.results.success).toBe(true);
  });
