...
class KGNodeDescriptionPipe(AsyncPipe):
    ...
    async def _run_logic(self, input: Input, state: AsyncState, run_id: uuid.UUID, *args: Any, **kwargs: Any) -> AsyncGenerator[Any, None]:
        ...
        async def process_entity(entity, triples):
            ...
            messages = [{'role':'user', 'content': summarization_content.format(entity_info=entity_info, description=entity.description, triples_txt=triples_txt)}]

            out_entity = self.kg_provider.retrieve_cache('entities_with_description', f'{entity.name}_{entity.category}')
            if out_entity:
                ...
                # embedding 
                description_embedding = await self.embedding_provider.async_get_embeddings([entity.description])
                entity.description_embedding = description_embedding[0]

                # name embedding
                name_embedding = await self.embedding_provider.async_get_embeddings([entity.name])
        ...
        self.kg_provider.upsert_entities(processed_entities, with_embeddings=True)
