...
class LocalPipeLoggingProvider(PipeLoggingProvider):
    def __init__(self, config: LoggingConfig):
        ...
        self.logging_path = config.logging_path or os.getenv("LOCAL_DB_PATH", "local.sqlite")
        if not self.logging_path:
            raise ValueError("Please set the environment variable LOCAL_DB_PATH.")
        self.conn = None
        try:
            import aiosqlite
            self.aiosqlite = aiosqlite
        except ImportError:
            raise ImportError("Please install aiosqlite to use the LocalPipeLoggingProvider.")
    ...
    async def log(self, pipe_run_id: uuid.UUID, key: str, value: str, is_pipeline_info=False):
        collection = self.log_info_table if is_pipeline_info else self.log_table
    ...
    async def get_run_ids(self, pipeline_type: Optional[str] = None, limit: int = 10) -> List[uuid.UUID]:
    ...
    async def get_logs(self, run_ids: List[uuid.UUID], limit_per_run_and_type: int = 10) -> list:
        if not run_ids:
        ...
        new_rows = []
        for row in rows:
            new_rows.append((row[0], uuid.UUID(row[1]), row[2], row[3], row[4]))
        return [{desc[0]: row[i] for i, desc in enumerate(cursor.description)} for row in new_rows]
...
class PostgresLoggingConfig(LoggingConfig):
    ...
    def validate(self) -> None:
        required_env_vars = ["POSTGRES_DBNAME", "POSTGRES_USER", "POSTGRES_PASSWORD", "POSTGRES_HOST", "POSTGRES_PORT"]
...
class PostgresPipeLoggingProvider(PipeLoggingProvider):
    def __init__(self, config: PostgresLoggingConfig):
        ...
        if not os.getenv("POSTGRES_DBNAME"):
            raise ValueError("Please set the environment variable POSTGRES_DBNAME.")
        if not os.getenv("POSTGRES_USER"):
            raise ValueError("Please set the environment variable POSTGRES_USER.")
        if not os.getenv("POSTGRES_PASSWORD"):
            raise ValueError("Please set the environment variable POSTGRES_PASSWORD.")
        if not os.getenv("POSTGRES_HOST"):
            raise ValueError("Please set the environment variable POSTGRES_HOST.")
        if not os.getenv("POSTGRES_PORT"):
            raise ValueError("Please set the environment variable POSTGRES_PORT.")
...
class PipeLoggingConnectionSingleton:
    ...
    @classmethod
    def configure(cls, logging_config: Optional[LoggingConfig] = LoggingConfig()):
        if not cls._is_configured:
            cls._config = logging_config
            cls._is_configured = True
        else:
            raise Exception("PipeLoggingConnectionSingleton is already configured.")

    @classmethod
    async def log(cls, pipe_run_id: uuid.UUID, key: str, value: str, is_pipeline_info=False):
        try:
            async with cls.get_instance() as provider:
                await provider.log(pipe_run_id, key, value, is_pipeline_info=is_pipeline_info)
        except Exception as e:
            logger.error(f"Error logging data: {e}")

    @classmethod
    async def get_run_ids(cls, pipeline_type: Optional[str] = None, limit: int = 10) -> List[uuid.UUID]:
        async with cls.get_instance() as provider:
            return await provider.get_run_ids(pipeline_type, limit)

    @classmethod
    async def get_logs(cls, run_ids: List[uuid.UUID], limit_per_run_and_type: int = 10) -> list:
        async with cls.get_instance() as provider:
