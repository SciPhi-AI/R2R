...
class PGVectorDB(VectorDBProvider):
    ...
    def _create_hybrid_search_function(self):
        ...
        """
        with self.vx.Session() as sess:
            sess.execute(text(drop_function_query))
        ...
        #     FROM vecs."{self.config.collection_name}"
        #     WHERE to_tsvector('english', metadata->>'text') @@ websearch_to_tsquery(query_text)
        #     AND (filter_condition IS NULL OR (metadata @> filter_condition::jsonb))
        #     ORDER BY rank_ix
        #     LIMIT LEAST(match_limit, 30) * 2
        # ),
        # semantic AS (
        ...
        #         ROW_NUMBER() OVER (ORDER BY vec <#> query_embedding) AS rank_ix
        #     FROM vecs."{self.config.collection_name}"
        #     WHERE filter_condition IS NULL OR (metadata @> filter_condition::jsonb)
        #     ORDER BY rank_ix
        #     LIMIT LEAST(match_limit, 30) * 2
        # )
        # SELECT
    ...
    #     # Construct the filter condition string for SQL
    #     filter_condition = None
    #     if filters:
    #         conditions = []
    ...
    # #             cast(:query_text as TEXT), cast(:query_embedding as VECTOR), cast(:match_limit as INT), 
    # #             cast(:full_text_weight as FLOAT), cast(:semantic_weight as FLOAT), cast(:rrf_k as INT),
    # #             cast(:filter_condition as TEXT)
    # #         )
    # #     """)

    # #     # Transform filters to SQL-compatible JSON condition
    # #     filter_condition = None
    # #     if filters:
    # #         filter_condition = json.dumps({k: {"$eq": v} for k, v in filters.items()})
    ...
    # #     # #         key: {"$eq": value} for key, value in filters.items()
    # #     # #     }
    # #     # #     filter_condition = json.dumps(mapped_filters)
    # #     # print('filter_condition = ', filter_condition)
    # #     # params = {
    # #     #     "query_text": str(query_text),
    # #     #     "query_embedding": list(query_vector),
