...
class PGVectorDB(VectorDBProvider):
    ...
    def _create_hybrid_search_function(self):
        ...
        #     WHERE metadata->>'text' @@ websearch_to_tsquery(query_text)
        #     ORDER BY rank_ix
        #     LIMIT LEAST(match_count, 30) * 2
        # ),
        # semantic AS (
        ...
        #     FULL OUTER JOIN semantic
        #         ON full_text.id = semantic.id
        #     JOIN vecs."{table_name}"
        #         ON COALESCE(full_text.id, semantic.id) = vecs."{table_name}".id
        #     ORDER BY
        #     COALESCE(1.0 / (rrf_k + full_text.rank_ix), 0.0) * full_text_weight +
    ...
    def hybrid_search(
        ...
        **kwargs
        ...
        #         :full_text_weight::FLOAT, :semantic_weight::FLOAT, :rrf_k::INT
        #     )
        # """)
        query = text("""
            SELECT * FROM hybrid_search(
                cast(:query_text as TEXT), cast(:query_embedding as VECTOR), cast(:match_count as INT), 
