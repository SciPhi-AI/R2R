...
def create_graph():
    ...
    documents_overview = client.documents_overview(limit=1000)['results']
    document_ids = [document.get("id") for document in documents_overview if document.get("kg_creation_status") in ["pending", "failed", "enrichment_failure"]]
    client.create_graph(document_ids = document_ids)
    wait_till_ready("kg_creation_status", "success")


def enrich_graph():
    print("Enriching graph...")
    client.enrich_graph()
    wait_till_ready("kg_creation_status", "enriched")
