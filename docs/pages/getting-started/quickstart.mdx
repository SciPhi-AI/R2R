...
We will start by ingesting a list of assorted document types to showcase search, RAG, and more advanced functionalities. The script powering this quickstart, [`r2r/examples/demo.py`](https://github.com/SciPhi-AI/R2R/blob/main/r2r/examples/demo.py), can be easily customized to fit your specific use case.
...
<Callout type="info" emoji="🐳">
   ...
   If you would prefer to do the heavy lifting with Docker instead then refer to the [installation instructions](/getting-started/installation?tab=Installing+with+Docker+%F0%9F%90%B3) and then proceed to the [client-server cookbook](/cookbooks/client-server).
</Callout>
...
```bash filename="bash" copy
python -m r2r.examples.quickstart search --query="What is shown on the user screen?"
```
...
```bash filename="bash" copy
python -m r2r.examples.quickstart evaluate
```
