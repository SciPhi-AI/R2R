...
This guide offers a step-by-step walkthrough of the core features provided by R2R using the R2R Python SDK. The framework includes methods as well as a REST API for document ingestion, RAG, evaluation, and supporting features like observability, analytics, and document management.
...
<summary>Ingesting Files</summary>
    To comprehensively demonstrate the RAG functionalities of the R2R framework, we must start by ingesting a realistic set of documents. Running the command below will parse, chunk, embed, and store a prepared list of sample files. The included file types cover `html`, `pdf`, `png`, `mp3`, and `txt` examples:
