...
The `R2RPipeFactory` is

 responsible for creating the pipes used in the `R2RApp`. Pipes are the core components that handle various stages of processing within the pipelines.
...
class R2RPipeFactoryWithMultiSearch(R2RPipeFactory):
    def create_search_pipe(self, *args, **kwargs):
        ...
        query_transform_pipe = kwargs.get("multi_query_transform_pipe_override", None) or R2RQueryTransformPipe(
