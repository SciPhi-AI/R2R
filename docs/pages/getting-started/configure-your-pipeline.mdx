...
The R2R library provides flexibility in customizing various aspects of the Retrieval-Augmented Generation (RAG) pipeline to suit your specific needs.

By default, RAG pipelines are configured with the R2R [`config.json`](https://github.com/SciPhi-AI/R2R/blob/main/config.json) file. You can override any fields in this configuration file with your own custom settings.
...
- **`qdrant`**: Integration with Qdrant, a high-performance vector similarity search engine. Good for large datasetes (> 1M embedding vectors) which require fast vector search.
- **`pgvector`**: Integration with PGVector, a vector similarity search extension for PostgreSQL. Good for moderate sized datasets with relational properties.
...
You can successfully serve the [R2R Demo](/getting-started/r2r-demo) using Qdrant the following command sequence:
...
You can successfully serve the [R2R Demo](/getting-started/r2r-demo) using local embeddings with rerank, as shown above, with the following command sequence:
...
### Ex. LLM Configuration

Any LLM supported by LiteLLM is immediately supported at runtime. For instance, to run with a local LLM you may serve via ollama and then pass the served model in directly as a generation config json payload. See the example below:
...
### Ex. Postgres Configuration

An example configuration specifying Qdrant as a vector database provider can be found in [`r2r/examples/data/example_postgres_logger_config.json`](https://github.com/SciPhi-AI/R2R/tree/main/r2r/examples/data/example_postgres_logger_config.json):
...
You can successfully serve the [R2R Demo](/getting-started/r2r-demo) using postgres logging using the following command sequence:
