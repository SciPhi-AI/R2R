...
An example configuration specifying Qdrant as a vector database provider can be found in [`r2r/examples/data/example_qdrant_config.json`](https://github.com/SciPhi-AI/R2R/tree/main/r2r/examples/data/example_qdrant_config.json). When R2R is ran with this configuration the `vector_database` settings on the default `config.json` are overriden:
