...
<LinkTabs items={['Installing with Pip 🐍', 'Installing with Docker 🐳']}>
  ...
  ```bash
  # use `'r2r[all]'` to download all required deps
  pip install r2r
...
Full installation is largely meant to be used by developers who wish to extend R2R.
...
The default (and currently only) vector database for R2R. Allows for larger scale workloads and benefits from relational properties:
