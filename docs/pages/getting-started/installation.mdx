...
For a quick setup, check out [fast install](#fast-install). You should use [full install](#full-install) if you're interested in leveraging the options offered by `config.json`.
...
### Fast Install: Quick Setup with `pip`
