...
**This document extends the [R2R Quickstart](/getting-started/quickstart)** by demonstrating how to set up and use the R2R with a Client-Server architecture. The R2R server can be stood up to handle requests, while the client can communicate with the server to perform various operations. The server API can be [viewed here](/getting-started/r2r-api).
