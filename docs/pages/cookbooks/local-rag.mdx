...
<Tabs items={['Installing with Pip 🐍', 'Installing with Docker 🐳']}>
  <Tabs.Tab>
    Running local RAG requires an additional `local-embedding` dependency to support embedding creation.
    ```bash
      pip install 'r2r[local-embedding]'

      # start the server, further commands must run in a separate termiinal
...
```bash filename="bash" copy
python -m r2r.examples.quickstart ingest_as_files --no-media=true --client_server_mode
```
