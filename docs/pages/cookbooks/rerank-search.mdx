...
This guide assumes R2R is already installed and the basic local RAG cookbook [located here](/cookbooks/local-rag) has been completed. To run R2R with local embedding and reranking, override the default `config.json` with settings like those in the collapsible below.
