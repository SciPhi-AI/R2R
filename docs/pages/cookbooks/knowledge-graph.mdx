...
We can execute the script above by running the following

```bash filename="User Terminal"
python -m python -m r2r.examples.scripts.basic_kg_cookbook
```


The script will also run the code below which directly demonstrates the relevant functionalities:
