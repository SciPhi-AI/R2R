...
Unlike other frameworks with too many layers of abstraction, with too many directions or that are unstable, we wanted to build a simple framework that allows developers to more easily `deploy`, `modify` and `maintain` production ready RAG pipelines.

In short, we aim to think and develop from first principles to set an industry standard.
...
- [Quick Install](#fast-install): Quick setup using `pip`.
