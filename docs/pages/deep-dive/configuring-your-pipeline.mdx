...
python -m r2r.examples.demo --config_path=$WORKDIR/r2r/examples/data/example_rerank_config.json serve
```
...
```bash filename="bash" copy
python -m r2r.examples.demo rag --rag_generation_config='{"model":"ollama/llama2"}' --query="Who was Aristotle?"
```
...
# Serve the demo with the custom user settings
export WORKDIR=YOUR_R2R_DIRECTORY
python -m r2r.examples.demo --config_path=$WORKDIR/r2r/examples/data/example_postgres_logger_config.json serve
```
