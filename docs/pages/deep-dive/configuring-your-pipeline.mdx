...
python -m r2r.examples.servers.configurable_pipeline --config=local_ollama_rerank
```
...
```bash filename="bash" copy
python -m r2r.examples.quickstart rag --rag_generation_config='{"model":"ollama/llama2"}' --query="Who was Aristotle?"
```
