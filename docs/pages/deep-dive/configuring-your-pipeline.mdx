...
R2R is configured via a simple `config.json`, with the defaults shown here (https://github.com/SciPhi-AI/R2R/blob/main/config.json). You can override any fields in this configuration file with your custom settings and pass in this new configuration to the R2R builder.
...
You can serve R2R using local embeddings with rerank with the following command sequence:
...
python -m r2r.examples.quickstart --config_name=local_ollama_rerank serve
```
...
```bash filename="bash" copy
python -m r2r.examples.quickstart rag --rag_generation_config='{"model":"ollama/llama2"}' --config_name=local_ollama --query="Who was Aristotle?"
```
...
```json filename="r2r/examples/configs/postgres_logging.json"
{
  "logging": {
    "provider": "postgres",
    "log_table": "logs",
...
You can serve the [R2R Quickstart](/getting-started/quickstart) using Postgres logging with the following command sequence:
...
# Serve the demo with the custom user settings
export WORKDIR=YOUR_R2R_DIRECTORY
python -m r2r.examples.quickstart --config_path=$WORKDIR/r2r/examples/data/example_postgres_logger_config.json serve
```
