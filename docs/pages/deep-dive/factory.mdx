...
- `sentence_transformers`: Returns an instance of `SentenceTransformerEmbeddingProvider` with the specified model.
...
7. Creates an instance of the `rag_pipeline_impl` (default: `BasicRAGPipeline`) with the language model, vector database, embedding model, embeddings provider, and logging connection.
