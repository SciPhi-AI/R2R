...
### `create_embedding_provider(embedding_config: dict[str, Any])`
...
1. Sets up logging based on the `logging` configuration.
2. Retrieves the embeddings provider using `create_embedding_provider` or uses the provided `embedding_provider`.
3. Retrieves the vector database using `create_vector_db_provider` or uses the provided `vector_db_provider`.
4. Initializes the vector database collection with the specified `collection_name` and `embedding_dimension`.
5. Retrieves the language model using `create_llm_provider` or uses the provided `llm`.
6. Creates a `PipeLoggingConnectionSingleton` instance for logging purposes.
...
12. Creates the end-to-end pipeline application using the `app_fn` (default: `default_app`) with the ingestion pipeline, embedding pipeline, RAG pipeline, evaluation pipeline, configuration, and logging connection.
