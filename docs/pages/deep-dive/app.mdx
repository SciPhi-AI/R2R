...
- `app_fn`: A function that creates the FastAPI application (default: `app_factory`).
...
The `app_factory` function, which is the default `app_fn` used by the factory, takes the created pipeline instances and the configuration as parameters and sets up the FastAPI application with the necessary endpoints and middleware.
...
You can add custom endpoints to the R2R application by modifying the `app_factory` function or providing a custom `app_fn` to the `E2EPipelineFactory.create_pipeline()` method.
