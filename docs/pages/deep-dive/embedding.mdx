...
The `BasicEmbeddingPipeline` is initialized with an `embedding_provider`, `vector_db_provider`, `text_splitter`, an optional `pipe_logger`, `embedding_batch_size`, and `id_prefix`.
