...
```bash
{'results': {'processed_documents': [{'id': '93123a68-d668-51de-8291-92162730dc87', 'title': 'aristotle.txt', ...}], 'failed_documents': [], 'skipped_documents': []}}
```
...
<Tab title="Curl">
```bash
curl -X POST http://localhost:8000/v2/documents_overview \
...
<Tab title="Python">
```python
client.document_chunks("93123a68-d668-51de-8291-92162730dc87")
```
</Tab>

<Tab title="JavaScript">
```javascript
await client.documentChunks("93123a68-d668-51de-8291-92162730dc87"),
```
</Tab>

<Tab title="Curl">
```bash
curl -X POST http://localhost:8000/v2/document_chunks \
  -H "Content-Type: application/json" \
  -d '{
    "document_id": "93123a68-d668-51de-8291-92162730dc87"
  }'
...
<Tab title="JavaScript">
```javascript
await client.delete(["document_id"], ["93123a68-d668-51de-8291-92162730dc87"]);
```
</Tab>
...
<Tab title="Curl">
```bash
curl -X DELETE http://localhost:8000/v2/delete \
  -H "Content-Type: application/json" \
  -d '{
    "keys": ["document_id"],
    "values": ["93123a68-d668-51de-8291-92162730dc87"]
  }'
```
</Tab>
</Tabs>
...
<Accordion icon="arrows-rotate" title="Update Documents">
  ...
  <Tabs>
    ...
    <Tab title="Curl">
    ```bash
    curl -X POST http://localhost:8000/v2/update_files \
...
<Warning>
Knowledge Graphs are not constructed by default, refer to the [cookbook here](/cookbooks/knowledge-graph) before attempting to run the command below!
</Warning>
...
await client.search("Who founded Airbnb?", true, {}, 10, false, {}, {
    useKgSearch: true,
    kgSearchType: "global",
    kgSearchLevel: "global",
    entityTypes: ["Person", "Organization"],
    relationships: ["founder", "CEO"],
    maxCommunityDescriptionLength: 16384,
...
- `kg_search_level`: Specify the level of search (e.g., "global", "community", "entity").
...
<Tab title="Curl">
```bash
curl -X POST http://localhost:8000/v2/verify_email/123456
```
</Tab>
...
<Tab title="Curl">
```bash
curl -X POST http://localhost:8000/v2/refresh_access_token \
...
```bash
curl -X POST http://localhost:8000/v2/users_overview \
  -H "Content-Type: application/json" \
  -d '{}'
```
</Tab>
