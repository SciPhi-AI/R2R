...
client = R2RClient("http://localhost:7272")
kg_search_settings = { 'run_type': 'estimate' }
...
{
  "results": {
    "message": "These are estimated ranges, actual values may vary. To run the KG creation process, run `create-graph` with `--run` in the cli, or `run_type=\"run\"` in the client.",
    "document_count": 2,
    "number_of_jobs_created": 3,
    "total_chunks": 29,
    "estimated_entities": "290 - 580",
...
# Then, you can run the graph creation process with:
kg_search_settings = { 'run_type': 'run' }
...
result = client.search(query, kg_search_settings=kg_search_settings)
print(result)
