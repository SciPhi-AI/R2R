...
vector_search_settings = {
    ...
    "selected_group_ids": ["c3291abf-8a4e-5d9d-80fd-232ef6fd8526"]
}
...
5. `selected_group_ids` (list[UUID]): Group IDs to search for
...
filters = {
    "$and": [
        {"metadata.publication_date": {"$gte": "2023-01-01"}},
        {"metadata.author": {"$in": ["John Doe", "Jane Smith"]}},
        {"metadata.category": {"$ilike": "%technology%"}}
    ]
}
