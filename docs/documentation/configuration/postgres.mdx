...
R2R uses Postgres as its default database provider for relational and vector search queries. This means that Postgres is invovled in handling authentication, document management, and search across R2R. For robust search, R2R uses the `pgvector` extension and `ts_rank` to implement [customizable hybrid search](cookbooks/hybrid-search).
