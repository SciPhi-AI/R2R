...
Postgres was selected to power R2R because it is free, open source, and widely considered be a stable production-ready database. Further, the Postgres community has implemented pgvector for efficient storage and retrieval of vector embeddings alongside traditional relational data.
...
export POSTGRES_VECS_COLLECTION=your_collection_name
```
