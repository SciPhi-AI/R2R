...
async def health(ctx: click.Context) -> None:
    ...
    try:
        ...
        click.echo(f"An unexpected error occurred while checking health: {e}", err=True)
        raise
...
async def settings(ctx: click.Context) -> None:
    ...
    try:
        ...
        click.echo(f"An unexpected error occurred while retrieving settings: {e}", err=True)
        raise
...
async def status(ctx: click.Context) -> None:
    ...
    try:
        ...
        click.echo(f"An unexpected error occurred while retrieving status: {e}", err=True)
        raise
...
@click.option("--full", is_flag=True, help="Run the full R2R compose? This includes Hatchet and Unstructured.")
@click.option("--project-name", default=None, help="Project name for Docker deployment")
@click.option("--config-name", default=None, help="Name of the R2R configuration to use")
@click.option("--config-path", default=None, help="Path to a custom R2R configuration file")
@click.option("--image", help="Docker image to use")
@click.option("--image-env", default="prod", help="Which dev environment to pull the image from?")
@click.option("--exclude-postgres", is_flag=True, default=False, help="Excludes creating a Postgres container in the Docker setup.")
async def serve(
    host: Optional[str],
    port: Optional[int],
    docker: bool,
    ...
    if full:
        click.echo("Running the full R2R setup which includes `Hatchet` and `Unstructured.io`.")

    if config_name in ["local_llm", "full_local_llm"]:
        click.secho("WARNING: The `local_llm` and `full_local_llm` configurations are deprecated and will be removed in a future release. Please use `ollama`, `full_ollama`, `lm_studio`, or `full_lm_studio` as your configuration file instead.", fg="yellow")
    ...
    if image and image_env:
        click.echo("WARNING: Both `image` and `image_env` were provided. Using `image`.")
    ...
    if not image and docker:
        ...
        def image_exists(img: str) -> bool:
            try:
                subprocess.run(["docker", "manifest", "inspect", img], check=True, capture_output=True, text=True)
                return True
        ...
        if image_exists(version_specific_image):
            ...
            click.echo(f"Neither {version_specific_image} nor {latest_image} found in remote registry. Please check the Docker images.", err=True)
            raise click.Abort()
    ...
    if docker:
        ...
        if "pytest" in sys.modules or "unittest" in sys.modules or os.environ.get("PYTEST_CURRENT_TEST"):
            ...
            if not wait_for_container_health(project_name or ("r2r-full" if full else "r2r"), "r2r"):
                click.secho("r2r container failed to become healthy.", fg="red")
                return
...
@cli.command()
@click.option("--volumes", is_flag=True, help="Remove named volumes declared in the `volumes` section of the Compose file")
@click.option("--remove-orphans", is_flag=True, help="Remove containers for services not defined in the Compose file")
@click.option("--project-name", default=None, help="Which Docker Compose project to bring down")
def docker_down(volumes: bool, remove_orphans: bool, project_name: Optional[str]) -> None:
    ...
    try:
        ...
        if result == 0:
            click.echo(f"{project_name} Docker Compose setup has been successfully brought down.")
        else:
            click.echo(f"An error occurred while bringing down the {project_name} Docker Compose setup. Attempting to remove the network...")
    except Exception as e:
...
def generate_report() -> None:
    ...
    try:
        ...
        docker_ps_output = subprocess.check_output(
            ["docker", "ps", "--format", "{{.ID}}\t{{.Names}}\t{{.Status}}"], text=True, timeout=5
        ).strip()
        report["docker_ps"] = [
        ...
        docker_network_output = subprocess.check_output(
            ["docker", "network", "ls", "--format", "{{.ID}}\t{{.Name}}"], text=True, timeout=5
        ).strip()
        networks = [
        ...
        report["docker_error"] = "Docker command not found. Is Docker installed and in PATH?"
    except subprocess.TimeoutExpired:
        report["docker_error"] = "Docker command timed out. Docker might be unresponsive."
...
def version() -> None:
    ...
    try:
        ...
        click.echo(f"An unexpected error occurred while retrieving the version: {e}", err=True)
        raise
