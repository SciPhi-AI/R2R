...
async def create(ctx: click.Context):
    ...
    try:
        ...
        click.echo(
            f"An unexpected error occurred while creating a conversation: {e}",
            err=True,
        )
...
@click.option(
    "--limit",
    default=100,
    help="The maximum number of conversations to return. Defaults to 100.",
)
@pass_context
async def list(ctx: click.Context, ids, offset, limit):
    ...
    try:
        ...
        click.echo(
            f"An unexpected error occurred while listing conversations: {e}",
            err=True,
        )
...
async def retrieve(ctx: click.Context, id):
    ...
    try:
        ...
        click.echo(
            f"Failed to retrieve conversation with ID '{id}': {str(e)}",
            err=True,
        )
    except Exception as e:
        click.echo(
            f"An unexpected error occurred while retrieving conversation with ID '{id}': {e}",
            err=True,
        )
...
async def delete(ctx: click.Context, id):
    ...
    try:
        ...
        click.echo(
            f"Failed to delete conversation with ID '{id}': {str(e)}", err=True
        )
    except Exception as e:
        click.echo(
            f"An unexpected error occurred while deleting conversation with ID '{id}': {e}",
            err=True,
        )
...
@click.option(
    "--limit",
    default=100,
    help="The maximum number of users to return. Defaults to 100.",
)
@pass_context
async def list_users(ctx: click.Context, id, offset, limit):
    ...
    try:
        with timer():
            response = await client.conversations.list_users(
        ...
        click.echo(
            f"Failed to list users for conversation ID '{id}': {str(e)}",
            err=True,
        )
    except Exception as e:
        click.echo(
            f"An unexpected error occurred while listing users for conversation ID '{id}': {e}",
            err=True,
        )
