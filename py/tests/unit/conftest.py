...
@pytest.fixture
async def global_cleanup(db_provider) -> None:
    """Global cleanup fixture that runs after all tests.
    Cleans up test data directly from database tables.
    ...
    try:
        ...
        # List of tables to clean up - order matters for foreign key constraints
        tables = [
            # Child tables first
            "chunks",
            "graphs_entities",
        ...
        for table in tables:
            try:
                ...
                print(f"Cleaned up table {project_name}.{table}")
