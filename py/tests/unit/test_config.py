...
from core.main.config import FUSEConfig


@pytest.fixture
def base_config():
    """Load the base fuse.toml config"""
    config_path = Path(__file__).parent.parent.parent / "fuse/fuse.toml"
...
def test_base_config_loading(base_config):
    ...
    assert config.ingestion.provider == "fuse"
...
def test_nested_config_preservation(merged_config):
    ...
    config = FUSEConfig(merged_config)
...
def get_config_files():
    ...
    return ["fuse.toml"] + [f.name for f in config_dir.glob("*.toml")]
...
def test_config_required_keys(config_file):
    ...
    if config_file == "fuse.toml":
        file_path = Path(__file__).parent.parent.parent / "fuse/fuse.toml"
    else:
        file_path = (
    ...
    config = FUSEConfig(config_data)

    # Test required sections
    for section in FUSEConfig.REQUIRED_KEYS:
        assert hasattr(config, section), f"Missing required section: {section}"

    # Test required keys in each section
    for section, required_keys in FUSEConfig.REQUIRED_KEYS.items():
...
def test_serialization_roundtrip(merged_config):
    ...
    roundtrip_config = FUSEConfig(toml.loads(serialized))
