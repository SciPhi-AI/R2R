...
@pytest.mark.asyncio
async def test_store_and_retrieve_file(postgres_file_provider):
    document_id = uuid.uuid4()
    file_name = "test_file.txt"
    file_content = io.BytesIO(b"Test file content")
    file_type = "text/plain"

    await postgres_file_provider.store_file(
        document_id, file_name, file_content, file_type
    )
    retrieved_file = await postgres_file_provider.retrieve_file(document_id)
...
@pytest.mark.asyncio
async def test_delete_file(postgres_file_provider):
    document_id = uuid.uuid4()
    file_name = "test_file.txt"
    file_content = io.BytesIO(b"Test file content")
    file_type = "text/plain"
    ...
    deleted = await postgres_file_provider.delete_file(document_id)

    assert deleted is True
    with pytest.raises(Exception):
        await postgres_file_provider.retrieve_file(document_id)


@pytest.mark.asyncio
async def test_get_files_overview(postgres_file_provider):
    ...
    files_overview = await postgres_file_provider.get_files_overview(limit=3)
    ...
    filtered_files_overview = await postgres_file_provider.get_files_overview(
