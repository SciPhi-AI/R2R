...
async def test_create_user(temporary_postgres_db_provider):
    user = await temporary_postgres_db_provider.relational.create_user(
...
async def test_get_user_by_id(temporary_postgres_db_provider):
    ...
    retrieved_user = (
        await temporary_postgres_db_provider.relational.get_user_by_id(user.id)
    )
...
async def test_get_user_by_email(temporary_postgres_db_provider):
    ...
    retrieved_user = (
        await temporary_postgres_db_provider.relational.get_user_by_email(
            "test@example.com"
        )
    )
    assert retrieved_user == user
...
async def test_delete_user(temporary_postgres_db_provider):
    ...
    await temporary_postgres_db_provider.relational.delete_user(user.id)
    user = await temporary_postgres_db_provider.relational.get_user_by_id(
        user.id
    )
    assert user is None
...
async def test_update_user(temporary_postgres_db_provider):
    ...
    result = await temporary_postgres_db_provider.relational.update_user(
        updated_user
    )
...
async def test_update_user_password(temporary_postgres_db_provider):
    ...
    await temporary_postgres_db_provider.relational.update_user_password(
...
async def test_get_all_users(temporary_postgres_db_provider):
    await temporary_postgres_db_provider.relational.create_user(
        "test1@example.com", "password"
    )
    await temporary_postgres_db_provider.relational.create_user(
        "test2@example.com", "password"
    )
    users = await temporary_postgres_db_provider.relational.get_all_users()
    assert len(users) >= 2
...
async def test_store_and_verify_verification_code(
    ...
    await temporary_postgres_db_provider.relational.store_verification_code(
        user.id, verification_code, expiry
    )
    await temporary_postgres_db_provider.relational.verify_user(
        verification_code
    )
    updated_user = (
        await temporary_postgres_db_provider.relational.get_user_by_id(user.id)
...
async def test_store_and_get_reset_token(temporary_postgres_db_provider):
    ...
    expiry = datetime.utcnow() + timedelta(hours=1)
    await temporary_postgres_db_provider.relational.store_reset_token(
        user.id, reset_token, expiry
    )
    user_id = await temporary_postgres_db_provider.relational.get_user_id_by_reset_token(
        reset_token
    )
...
async def test_add_and_remove_user_from_collection(
    ...
    await temporary_postgres_db_provider.relational.add_user_to_collection(
    ...
    await temporary_postgres_db_provider.relational.remove_user_from_collection(
        user.id, collection_id
    )
    updated_user = (
        await temporary_postgres_db_provider.relational.get_user_by_id(user.id)
