...
async def test_create_collection(temporary_postgres_db_provider):
    collection = (
        await temporary_postgres_db_provider.relational.create_collection(
...
async def test_get_collection(temporary_postgres_db_provider):
    ...
    retrieved_collection = (
        await temporary_postgres_db_provider.relational.get_collection(
...
async def test_update_collection(temporary_postgres_db_provider):
    ...
    updated_collection = (
        await temporary_postgres_db_provider.relational.update_collection(
            created_collection.collection_id,
...
async def test_delete_collection(temporary_postgres_db_provider):
    ...
    await temporary_postgres_db_provider.relational.delete_collection(
        created_collection.collection_id
    )
...
async def test_list_collections(temporary_postgres_db_provider):
    ...
    collections = (
        await temporary_postgres_db_provider.relational.list_collections()
    )
...
async def test_get_collections_by_ids(temporary_postgres_db_provider):
    ...
    collections = (
        await temporary_postgres_db_provider.relational.get_collections_by_ids(
...
async def test_assign_and_remove_document_from_collection(
    ...
    await temporary_postgres_db_provider.relational.assign_document_to_collection(
        document_id, collection.collection_id
    )
    document_collections = (
        await temporary_postgres_db_provider.relational.document_collections(
            document_id
        )
    )
    ...
    await temporary_postgres_db_provider.relational.remove_document_from_collection(
        document_id, collection.collection_id
    )
    document_collections = (
        await temporary_postgres_db_provider.relational.document_collections(
...
async def test_get_collections_for_user(temporary_postgres_db_provider):
    ...
    user_collections = await temporary_postgres_db_provider.relational.get_collections_for_user(
        user.id
    )
