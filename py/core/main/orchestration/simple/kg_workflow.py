...
def simple_kg_factory(service: KgService):
    ...
    async def enrich_graph(input_data):
        ...
        if workflow_status == KGEnrichmentStatus.SUCCESS:
            raise R2RException(
                "Communities have already been built for this collection. To build communities again, first reset the graph by submitting a POST request to `graphs/{collection_id}/reset`.",
                400,
            )
