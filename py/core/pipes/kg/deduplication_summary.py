...
class KGEntityDeduplicationSummaryPipe(AsyncPipe):
    ...
    async def _merge_entity_descriptions_llm_prompt(self, entity_name: str, entity_descriptions: list[str], generation_config: GenerationConfig) -> str:
    ...
    async def _merge_entity_descriptions(self, entity_name: str, entity_descriptions: list[str], generation_config: GenerationConfig) -> str:
        
        # TODO: Expose this as a hyperparameter
        if len(entity_descriptions) <= 5:
            return "\n".join(entity_descriptions)
        else:
            return await self._merge_entity_descriptions_llm_prompt(entity_name, entity_descriptions, generation_config)
    ...
    async def _prepare_and_upsert_entities(self, entities_batch: list[dict], collection_id: str) -> list[dict]:
        ...
        result = await self.kg_provider.add_entities(entities_batch, entity_table_name="entity_deduplicated", conflict_columns=["name", "collection_id"])

        logger.info(f"Upserted {len(entities_batch)} entities for collection {collection_id}")
    ...
    async def _run_logic(self, input: AsyncPipe.Input, state: AsyncState, run_id: UUID, *args: Any, **kwargs: Any) -> AsyncGenerator[dict, None]:
        ...
        logger.info(f"Running kg_entity_deduplication_summary for collection {collection_id} with settings kg_entity_deduplication_type: {kg_entity_deduplication_type}, kg_entity_deduplication_prompt: {kg_entity_deduplication_prompt}, generation_config: {generation_config}")

        entities = (await self.kg_provider.get_entities(
            collection_id, 
            offset, 
            limit, 
        ...
        logger.info(f"Retrieved {len(entities)} entities for collection {collection_id}")

        entity_names = [entity.name for entity in entities]

        logger.info(f"Retrieved {len(entity_names)} entity names for collection {collection_id}")

        entity_descriptions = (await self.kg_provider.get_entities(
            collection_id, 
            offset, 
            limit, 
        ...
        logger.info(f"Retrieved {len(entity_descriptions)} entity descriptions for collection {collection_id}")
        ...
        logger.info(f"Merging entity descriptions for collection {collection_id}")

        tasks = []
        for entity in entities:
            tasks.append(self._merge_entity_descriptions(entity.name, entity_descriptions_dict[entity.name], generation_config))

        logger.info(f"Merged {len(tasks)} entity descriptions for collection {collection_id}")
