...
class PostgresPromptsHandler:
    ...
    def __init__(
        self, project_name, connection_manager, prompt_directory=None
    ):
        ...
        for yaml_file in yaml_files:
            try:
                ...
                for prompt_data in yaml_data["prompts"]:
                    ...
                    preserve_existing = prompt_data.get(
                        "preserve_existing", False
                    )

                    if not name or not template:
                        self.logger.warning(
                            f"Invalid prompt data in {yaml_file}: {prompt_data}"
                        )
                        continue
                ...
                self.logger.error(
                    f"Error loading prompts from {yaml_file}: {e}"
                )
    ...
    async def add_prompt(
        self, name, template, input_types, preserve_existing=False
    ):
        """Add or update a prompt."""
        # Check if prompt already exists and should be preserved
        if name in self.prompts and preserve_existing:
            self.logger.info(
                f"Prompt '{name}' already exists and preserve_existing=True, skipping update"
            )
            return
    ...
    async def update_prompt(self, name, template=None, input_types=None):
        ...
        new_input_types = (
            input_types if input_types is not None else current["input_types"]
        )
    ...
    async def delete_prompt(self, name):
        ...
        query = (
            f"DELETE FROM {self._get_table_name('prompts')} WHERE name = $1"
        )
    ...
    async def get_cached_prompt(
        ...
        # If prompt_override is provided, just return it directly
        ...
        for key, _type_name in expected_input_types.items():
            if key not in inputs:
                raise ValueError(
                    f"Missing required input '{key}' for prompt '{prompt_name}'"
                )
        ...
        try:
            ...
            raise ValueError(f"Missing input in format string: {e}") from e
        except Exception as e:
            raise ValueError(
                f"Error formatting prompt '{prompt_name}': {e}"
            ) from e
