...
def _process_filter_dict(
    ...
    for key, value in filter_dict.items():
        ...
        if key == FilterOperator.AND:
            ...
            sub_conditions = [
                 _process_filter_dict(item, param_helper, top_level_columns, json_column)
            ...
            if sub_conditions:
                ...
                conditions.append(" AND ".join(f"({sc})" for sc in sub_conditions))
            ...
            if sub_conditions:
                 # Wrap individual sub-conditions in parens for clarity if joining multiple
                conditions.append(" OR ".join(f"({sc})" for sc in sub_conditions))
    ...
    if len(final_conditions) > 1:
        ...
        return final_conditions[0] # Return the single condition without extra parens
...
def _process_field_condition(
    ...
    is_top_level_standard_col = field in top_level_columns and field != json_column
    ...
    if field == json_column:
        ...
        is_metadata_target = True
        relative_path = field[len(json_column) + 1:] # Get path part after "metadata."
    elif not is_top_level_standard_col and not is_collection_id_shorthand and not is_collection_ids_field:
    ...
    if is_collection_id_shorthand:
        ...
        if isinstance(condition_spec, dict) and len(condition_spec) == 1:
             ...
             if op == FilterOperator.EQ: # Map $eq on shorthand to overlap check
                 return _build_collection_ids_condition("collection_ids", FilterOperator.OVERLAP, [value], param_helper)
             elif op == FilterOperator.NE: # Map $ne on shorthand to NOT overlap check (tricky, usually means "doesn't contain this one ID")
                 ...
                 return _build_collection_ids_condition("collection_ids", op, value, param_helper)
            ...
            return _build_collection_ids_condition("collection_ids", FilterOperator.OVERLAP, [condition_spec], param_helper)
        else:
        ...
        if relative_path:
            ...
            return _build_metadata_condition(relative_path, condition_spec, param_helper, json_column)
        else:
            ...
            if not isinstance(condition_spec, dict):
                raise FilterError(f"Filter for '{json_column}' column must be a dictionary specifying paths and conditions.")
            ...
            for meta_path, meta_condition_spec in condition_spec.items():
                ...
                condition_sql = _build_metadata_condition(meta_path, meta_condition_spec, param_helper, json_column)
                if condition_sql != "TRUE":
                    metadata_conditions.append(condition_sql)

            if not metadata_conditions: return "TRUE"
            if len(metadata_conditions) == 1: return metadata_conditions[0]
...
def _build_array_literal(items: List[Any], param_helper: ParamHelper, array_type: str) -> str:
...
def _build_collection_ids_condition(
    ...
    if target_column != "collection_ids":
         raise FilterError(f"Internal Error: _build_collection_ids_condition called with target '{target_column}'")
    ...
    if op in [FilterOperator.OVERLAP, FilterOperator.ARRAY_CONTAINS, FilterOperator.IN, FilterOperator.NIN]:
        ...
        array_literal = _build_array_literal(uuid_strings, param_helper, "uuid")
    ...
    elif op == FilterOperator.EQ: # Check if array IS EXACTLY this single element array
    ...
    elif op == FilterOperator.NE: # Check if array IS NOT EXACTLY this single element array
         if isinstance(value, (str, uuid.UUID)):
             try:
                 uuid_str = str(uuid.UUID(str(value)))
                 placeholder = param_helper.add(uuid_str)
...
def _build_metadata_condition(
    relative_path: str, condition_spec: Any, param_helper: ParamHelper, json_column: str
) -> str:
    """
    ...
    if isinstance(condition_spec, dict) and len(condition_spec) == 1:
        ...
        if key.startswith("$"):
            ...
            op = key
            val = value
            return _build_metadata_operator_condition(relative_path, op, val, param_helper, json_column)
        else:
            ...
            return _build_metadata_condition(new_relative_path, value, param_helper, json_column)
    else:
        # It's a direct value comparison (shorthand for EQ) like "value"
        # Apply EQ operator to the relative_path
        return _build_metadata_operator_condition(relative_path, FilterOperator.EQ, condition_spec, param_helper, json_column)


def _build_metadata_operator_condition(
    relative_path: str, op: str, value: Any, param_helper: ParamHelper, json_column: str
) -> str:
    """Builds the specific SQL for an operator on a JSONB path."""

    # Build JSON path expression based on relative_path
    path_parts = relative_path.split('.')
    use_text_extraction = op not in [FilterOperator.JSON_CONTAINS] # Use ->> or #>> for non-JSONB ops

    if len(path_parts) == 1:
        # Top-level key within the JSONB column
        json_accessor = f"{json_column} ->> '{path_parts[0]}'" if use_text_extraction else f"{json_column} -> '{path_parts[0]}'"
    else:
        ...
        json_accessor = f"{json_column} #>> '{path_literal}'" if use_text_extraction else f"{json_column} #> '{path_literal}'"
    ...
    if value is None:
        ...
        null_check_accessor = f"{json_column} -> '{path_parts[0]}'" if len(path_parts) == 1 else f"{json_column} #> '{path_literal}'"
    ...
    if op == FilterOperator.JSON_CONTAINS:
        try:
            ...
            raise FilterError(f"Value for '{FilterOperator.JSON_CONTAINS}' on '{relative_path}' must be JSON serializable: {e}")

    # --- Standard comparisons (operating on text extraction ->> or #>>) ---
    # Ensure json_accessor uses text extraction here
    if not use_text_extraction: # Should not happen based on logic above, but defensive check
         raise FilterError(f"Internal Error: Text extraction required for operator '{op}'")
    ...
    # --- String Like ---
        ...
        if not isinstance(value, str):
            raise FilterError(f"'{FilterOperator.LIKE}' requires a string value for metadata field '{relative_path}'.")
        placeholder = param_helper.add(value)
        return f"{json_accessor} LIKE {placeholder}"
    elif op == FilterOperator.ILIKE:
        if not isinstance(value, str):
            raise FilterError(f"'{FilterOperator.ILIKE}' requires a string value for metadata field '{relative_path}'.")
        placeholder = param_helper.add(value)
    ...
    # --- IN / NOT IN (operating on text extraction) ---
        ...
        if not isinstance(value, list):
             raise FilterError(f"'{FilterOperator.IN}' requires a list value for metadata field '{relative_path}'.")
        ...
        return f"({json_accessor}) IN ({', '.join(placeholders)})"
         ...
         if not isinstance(value, list):
             raise FilterError(f"'{FilterOperator.NIN}' requires a list value for metadata field '{relative_path}'.")
        ...
        raise FilterError(f"Unsupported operator '{op}' for metadata field '{relative_path}'.")
