...
def _process_field_condition(
    ...
    if not isinstance(condition, dict):
        return _build_operator_condition(
            field, FilterOperator.EQ, condition, params, top_level_columns, json_column
        )
    
    # Operator-based condition
    if len(condition) != 1:
        raise FilterError(f"Condition for field {field} must have exactly one operator")
    ...
    return _build_operator_condition(field, op, val, params, top_level_columns, json_column)
...
def _build_parent_id_condition(op: str, value: Any, params: list[Any]) -> tuple[str, list[Any]]:
    ...
    if op == FilterOperator.EQ:
        ...
        if not isinstance(value, list):
            raise FilterError("$in for parent_id expects a list of UUID strings")
        params.append(value)
        return f"parent_id = ANY(${param_idx}::uuid[])", params
    
    elif op == FilterOperator.NIN:
        if not isinstance(value, list):
            raise FilterError("$nin for parent_id expects a list of UUID strings")
        params.append(value)
...
def _build_collection_id_condition(op: str, value: Any, params: list[Any]) -> tuple[str, list[Any]]:
    """Build SQL condition for collection_id field (shorthand for collection_ids array)."""
    param_idx = len(params) + 1
    
    if op == FilterOperator.EQ:
        if not isinstance(value, str):
            raise FilterError("$eq for collection_id expects a single UUID string")
        params.append([value])
        return f"collection_ids && ${param_idx}::uuid[]", params
    
    elif op == FilterOperator.NE:
        if not isinstance(value, str):
            raise FilterError("$ne for collection_id expects a single UUID string")
        params.append([value])
        return f"NOT (collection_ids && ${param_idx}::uuid[])", params
    
    elif op == FilterOperator.IN:
        if not isinstance(value, list):
            raise FilterError("$in for collection_id expects a list of UUID strings")
        params.append(value)
        return f"collection_ids && ${param_idx}::uuid[]", params
    
    elif op == FilterOperator.NIN:
        if not isinstance(value, list):
            raise FilterError("$nin for collection_id expects a list of UUID strings")
        params.append(value)
        return f"NOT (collection_ids && ${param_idx}::uuid[])", params
        ...
        if isinstance(value, str):
            ...
            raise FilterError("$contains for collection_id expects a UUID or list of UUIDs")
        return f"collection_ids @> ${param_idx}::uuid[]", params
...
def _build_collection_ids_condition(op: str, value: Any, params: list[Any]) -> tuple[str, list[Any]]:
    """Build SQL condition for collection_ids field (array of UUIDs)."""
    param_idx = len(params) + 1
    
    if op == FilterOperator.EQ:
        if not isinstance(value, list):
            raise FilterError("$eq for collection_ids expects a list of UUID strings")
        params.append(value)
        return f"collection_ids = ${param_idx}::uuid[]", params
        ...
        if isinstance(value, str):
            ...
            raise FilterError("$contains for collection_ids expects a UUID or list of UUIDs")
        return f"collection_ids @> ${param_idx}::uuid[]", params
...
def _build_column_condition(field: str, op: str, value: Any, params: list[Any]) -> tuple[str, list[Any]]:
...
def _build_metadata_condition(
    ...
    if op == FilterOperator.IN or op == FilterOperator.CONTAINS or isinstance(value, (list, dict)):
    ...
    if op == FilterOperator.EQ:
        ...
        if use_text_extraction:
            str_vals = [str(v) if isinstance(v, (int, float)) else v for v in value]
