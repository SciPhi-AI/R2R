...
def _build_parent_id_condition(
    ...
    if op == FilterOperator.EQ:
        ...
        try:
            ...
            if all_uuids:
                params.append(uuids)
                return "parent_id = ANY(?::uuid[])", params
            else:
                # For non-UUID strings, use text array
                params.append(value)
                return "parent_id = ANY(?::text[])", params
        ...
        try:
            ...
            if all_uuids:
                params.append(uuids)
                return "parent_id != ALL(?::uuid[])", params
            else:
                # For non-UUID strings, use text array
                params.append(value)
                return "parent_id != ALL(?::text[])", params
        except Exception:
...
def _build_collection_id_condition(
    ...
    if op == FilterOperator.EQ:
        ...
        try:
            ...
            params.append(value_str)
            return "collection_ids @> ARRAY[?::text]", params

    elif op == FilterOperator.NE:
        if value is None:
            return (
                "collection_ids IS NOT NULL AND collection_ids != '{}'",
                params,
            )
        ...
        try:
            ...
            # For non-UUID strings (in tests)
            params.append(value_str)
            return "NOT (collection_ids @> ARRAY[?::text])", params
...
def _build_collection_ids_condition(
    ...
    # Handle other operators (IN, NIN, etc.)
        ...
        return (
            f"collection_ids && ARRAY[{', '.join(placeholder_parts)}]::text[]",
            params,
        )
        ...
        return (
            f"NOT (collection_ids && ARRAY[{', '.join(placeholder_parts)}]::text[])",
            params,
        )
...
def _build_metadata_condition(
    ...
    # Handle array operators
        ...
        # Handle empty lists specially - return a FALSE condition
    ...
    # Handle JSON operators
        ...
        if op == FilterOperator.CONTAINS:
            ...
            # Don't call json.dumps on the value to avoid adding extra quotes
            params.append(value)
            return (
...
def build_full_text_search(
    query: str, params: list[Any], json_column="metadata"
) -> tuple[str, list[Any]]:
    """
