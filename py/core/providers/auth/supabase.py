...
class SupabaseAuthProvider(AuthProvider):
    ...
    def __init__(
        ...
        self.supabase_url = config.extra_fields.get(
            "supabase_url", None) or os.getenv("SUPABASE_URL")
        self.supabase_key = config.extra_fields.get(
            "supabase_key", None) or os.getenv("SUPABASE_KEY")
        if not self.supabase_url or not self.supabase_key:
            raise HTTPException(
                status_code=500,
                detail="Supabase URL and key must be provided",
            )
        self.supabase: Client = create_client(self.supabase_url,
                                              self.supabase_key)
    ...
    async def register(
        ...
        if user := self.supabase.auth.sign_up(email=email, password=password):
            raise R2RException(
                status_code=400,
                message=
                "Supabase provider implementation is still under construction",
            )
        else:
    ...
    async def verify_email(self, email: str,
        ...
        if response := self.supabase.auth.verify_email(email,
                                                       verification_code):
    ...
    async def login(self, email: str, password: str) -> dict[str, Token]:
        ...
        if response := self.supabase.auth.sign_in(email=email,
                                                  password=password):
    ...
    async def refresh_access_token(self,
        ...
        if response := self.supabase.auth.refresh_access_token(refresh_token):
            ...
            return {
                "access_token":
                Token(token=new_access_token, token_type="access"),
                "refresh_token":
                Token(token=new_refresh_token, token_type="refresh"),
            }
        else:
            raise R2RException(status_code=401,
    ...
    async def change_password(self, user: User, current_password: str,
        ...
        if response := self.supabase.auth.update(user.id,
