...
# def resize_base64_image(base64_string: str, max_size: Tuple[int, int] = (1568, 1568),
#                        max_megapixels: float = 1.15) -> str:
#     """
#     Resize a base64 encoded image to fit within the specified dimensions and megapixel limit.

#     Args:
#         base64_string: Base64 encoded image string
#         max_size: Maximum (width, height) in pixels
#         max_megapixels: Maximum size in megapixels (width × height)

#     Returns:
#         Resized base64 encoded image string, same media_type
#     """
#     print("RESIZING NOW!!!")
#     print("-"*100)
...
#         return base64_string  # Return original if we can't process it

#     width, height = image.size
#     current_megapixels = (width * height) / 1_000_000

#     # Check if resizing is needed
#     if (width <= max_size[0] and height <= max_size[1] and
#         current_megapixels <= max_megapixels):
#         return base64_string  # No resizing needed

#     # Calculate new dimensions to maintain aspect ratio
#     if current_megapixels > max_megapixels:
#         # Scale down to max megapixels
#         scale_factor = (max_megapixels / current_megapixels) ** 0.5
#         new_width = int(width * scale_factor)
#         new_height = int(height * scale_factor)

#         # Make sure we're still within max dimensions
#         if new_width > max_size[0]:
#             scale_factor = max_size[0] / new_width
#             new_width = max_size[0]
#             new_height = int(new_height * scale_factor)

#         if new_height > max_size[1]:
#             scale_factor = max_size[1] / new_height
#             new_height = max_size[1]
#             new_width = int(new_width * scale_factor)
#     else:
#         # Just scale down to max dimensions
#         scale_width = max_size[0] / width if width > max_size[0] else 1
#         scale_height = max_size[1] / height if height > max_size[1] else 1
...
#     original_tokens = estimate_image_tokens(width, height)
#     new_tokens = estimate_image_tokens(new_width, new_height)

#     logger.info(f"Resized image from {width}x{height} ({current_megapixels:.2f} MP, ~{original_tokens} tokens) to "
#                 f"{new_width}x{new_height} ({(new_width * new_height / 1_000_000):.2f} MP, ~{new_tokens} tokens)")
